...
- 每个仓库同一时间最多运行一个同步任务。
- 同步任务使用独立的同步互斥锁串行执行；仓库写入锁只串行化 App 管理写入、最近使用写入和 Git 的本地状态修改。App 与 QQ 的只读路径不得获取该锁，Git `status/add/commit/rebase`、冲突恢复或失败都不能阻塞已有内容读取。
- 所有 JGit 仓库打开、校验、克隆和同步只能从 `Dispatchers.IO` 或 WorkManager 后台线程调用；App 状态对象构造和 Compose 主线程不得调用 `Git.open()` 或等待 Git Future。
- 根索引、包内索引和最近使用读取采用原子文件的只读候选，并缓存最近一次完整解析结果。Git 中间态、冲突标记、文件替换窗口或写入失败时返回最近一次有效结果；Git 完成后的协议校验必须绕过缓存直接检查真实工作树，不能用旧缓存掩盖损坏。
...
- 修复后同步锁继续串行化完整 Git 流水线，当时的内容锁（现写入锁）只覆盖本地提交、rebase、冲突处理和协议校验；fetch、push 网络阶段不持有该锁。fetch 后再次提交等待期间产生的本地变化，再进入 rebase。
- JVM 并发测试在 fetch 开始后从另一线程取得写入锁并写入文件，随后验证该文件被补提交并推送。`CPH2609` 真机在 fetch 已等待约 16 秒时再次打开“添加到 EmoRepo”，Provider `packs` 查询约 7 ms 完成，窗口与预览约 2.55 秒内显示；后续 GitHub 连接失败不影响窗口，日志没有 IPC 超时、ANR 或崩溃。

## 2026-09-02 Git 本地写入与只读隔离回归

- `EmoRepoState` 构造阶段不再调用 `Git.open()`；主线程只检查仓库和 `.git` 目录是否存在，完整 JGit 有效性校验继续在 `Dispatchers.IO` 中完成。
- 表情包与最近使用读取不再获取仓库写入锁。JVM 并发测试在 Git `STAGE` 已持锁时于 1 秒内同时读取表情包和最近使用；原子文件替换窗口读取旧备份且不改名、不删除写入侧文件。
- 写入开始前保留最近一次有效表情包快照，并以奇偶写入代次识别并发窗口。模拟 Git 将根索引写坏并失败时，普通读取继续返回旧快照；写入期结束后的静态协议损坏仍由绕过缓存的严格校验报错。
- `CPH2609` 覆盖安装后冷启动完整执行预检、状态、fetch、补提交、rebase、协议校验和 push，最终同步成功；同期主页持续显示 9 个表情包，进程没有 ANR 或崩溃。
