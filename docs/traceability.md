...
| HOOK-005 | QQ 面板将折叠包连续置于分页末尾、在同一包栏内展开，并支持网格到底后继续上拖循环切包 | `architecture/qq-panel.md` | implemented | `PanelNavigation`、`PackTabAdapter`、`EndOfPackPullDetector`、`PanelNavigationTest`；最近→下一包及末包→最近循环真机通过，真实折叠包同栏展开待验收 |
