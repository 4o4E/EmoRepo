...
| HOOK-005 | QQ 面板将折叠包连续置于分页末尾并在同一包栏内展开；横向分页首尾循环且恢复各包网格滚动位置 | `architecture/qq-panel.md` | superseded | 已由 HOOK-008 的单一纵向连续容器取代，历史真机结果保留在专题文档 |
...
| IMAGE-004 | QQ 面板首帧、网格动画和长按预览不得因解码器、有损缓存、动画降采样或帧合成而异常 | `architecture/qq-panel.md` | verified | `QqPanelFirstFrameCache` v3 `ImageDecoder+sRGB`、网格/动态封面原尺寸解码、长按 1024 px 解码、`preserveImageColors`；131 项 JVM 测试、Lint/Debug 构建，“薯条”网格双帧、长按双帧和松手关闭真机回归 |
